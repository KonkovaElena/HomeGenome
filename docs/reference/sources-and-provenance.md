...
| `docs/reference/case-export-bundle-contract.md` | reference | repo-authored implementation contract for portable case exports | `cleared` | Описывает уже реализованный в control plane экспортный контракт без внешних нереплицируемых claims. |
| `docs/reference/homegenome-control-plane.openapi.yaml` | reference | repo-authored API contract for current control-plane surfaces | `cleared` | Фиксирует минимальный внешний API слой (snapshot/export bundle) без overclaiming про full GA4GH server coverage. |
| `docs/reference/standards-adoption-matrix-2026-04-22.md` | reference | repo-authored mapping from SOTA standards to HomeGenome implementation status | `cleared` | Фиксирует какие рекомендации уже реализованы, а какие оставлены в planned backlog. |
